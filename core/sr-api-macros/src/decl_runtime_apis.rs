...
impl<'a> ToClientSideDecl<'a> {
	fn fold_item_trait_items(&mut self, items: Vec<TraitItem>) -> Vec<TraitItem> {
		...
		items.into_iter().for_each(|i| match i {
			TraitItem::Method(method) => {
				let (fn_decl, fn_impl, fn_decl_ctx, fn_impl_ctx) = self.fold_trait_item_method(method.clone());
				result.push(fn_decl.into());
				result.push(fn_decl_ctx.into());

				if let Some(fn_impl) = fn_impl {
					result.push(fn_impl.into());
					result.push(fn_impl_ctx.unwrap().into());
				}
			},
			r => result.push(r),
		});
	...
	fn fold_trait_item_method(&mut self, method: TraitItemMethod)
		-> (TraitItemMethod, Option<TraitItemMethod>, TraitItemMethod, Option<TraitItemMethod>) {
		...
		let (fn_decl_ctx, fn_impl_ctx) = self.create_method_with_context(method);
		(fn_decl, fn_impl, fn_decl_ctx, fn_impl_ctx)
	}

	fn create_method_with_context(&mut self, method: TraitItemMethod) -> (TraitItemMethod, Option<TraitItemMethod>) {
		let crate_ = self.crate_;
		let context_arg: syn::FnArg = parse_quote!( context: #crate_::runtime_api::ExecutionContext );

		let mut method_ctx = method.clone();
		method_ctx.sig.ident = Ident::new(&format!("{}_with_context", &method_ctx.sig.ident), Span::call_site());

		// Declaration.
		let mut fn_decl_ctx = self.create_method_decl(method_ctx.clone(), quote!( context ));
		fn_decl_ctx.sig.decl.inputs.insert(2, context_arg.clone());

		// Implementation.
		let mut fn_impl_ctx = self.create_method_runtime_api_impl(method_ctx.clone());
		if let Some(ref mut fn_impl_ctx_) = fn_impl_ctx {
			fn_impl_ctx_.sig.decl.inputs.insert(2, context_arg.clone());
		}

		(fn_decl_ctx, fn_impl_ctx)
	}
